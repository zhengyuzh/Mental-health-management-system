...
!function (i, d) {
    var e = i.layui && layui.define, f, h, j = {
        getPath: function () {
            var m = document.currentScript ? document.currentScript.src : function () {
                var p = document.scripts, o = p.length - 1, q;
                for (var n = o; n > 0; n--) {
                    if (p[n].readyState === "interactive") {
                        q = p[n].src;
                        break
                    }
                }
        ...
        link: function (l, t, m) {
            ...
            (function q(v) {
                var w = 100, x = document.getElementById(n);
                if (++r > 10 * 1000 / w) {
                    return i.console && console.error(o + ".css: Invalid")
                }
                if (parseInt(j.getStyle(x, "width")) === 1989) {
                    if (v === p) {
                        x.removeAttribute("lay-status")
                    }
                    x.getAttribute("lay-status") === p ? setTimeout(q, w) : t()
                } else {
                    x.setAttribute("lay-status", p);
                    setTimeout(function () {
                        q(p)
                    }, w)
                }
    ...
    var b = function (m) {
        ...
        document.body ? l() : setTimeout(function () {
            l()
        }, 30)
    };
    b.pt = b.prototype;
    ...
    b.pt.creat = function () {
        ...
        m.type == 4 ? o.tips() : function () {
            o.offset();
            parseInt(j.getStyle(document.getElementById(c.MOVE), "z-index")) || function () {
    ...
    b.pt.callback = function () {
        ...
        if (m.shadeClose) {
            o.shadeo.on("click", function () {
                g.close(o.index)
            })
        }
        l.find(".layui-layer-min").on("click", function () {
            var p = m.min && m.min(l, o.index);
    ...
    g.min = function (o, n) {
        n = n || {};
        var m = f("#" + c[0] + o), s = f("#" + c.SHADE + o), r = m.find(c[1]).outerHeight() || 0,
            q = m.attr("minLeft") || (181 * j.minIndex) + "px", l = m.css("position"),
            p = {width: 180, height: r, position: "fixed", overflow: "hidden"};
        j.record(m);
        if (j.minLeft[0]) {
            q = j.minLeft[0];
            j.minLeft.shift()
    ...
    g.restore = function (m) {
        var l = f("#" + c[0] + m), p = f("#" + c.SHADE + m), o = l.attr("area").split(","), n = l.attr("type");
        g.style(m, {
    ...
    g.photos = function (w, q, s) {
        var n = {};
        w = w || {};
        if (!w.photos) {
            return
        }
        var v = !(typeof w.photos === "string" || w.photos instanceof f), u = v ? w.photos : {}, p = u.data || [],
            m = u.start || 0;
        n.imgIndex = (m | 0) + 1;
        w.img = w.img || "img";
        var t = w.success;
        delete w.success;
        if (!v) {
            var r = f(w.photos), o = function () {
                p = [];
                r.find(w.img).each(function (x) {
                    var y = f(this);
        ...
        l(p[m].src, function (x) {
            ...
            n.index = g.open(f.extend({
                ...
                content: '<div class="layui-layer-phimg">' + '<img src="' + p[m].src + '" alt="' + (p[m].alt || "") + '" layer-pid="' + p[m].pid + '">' + function () {
                    if (p.length > 1) {
